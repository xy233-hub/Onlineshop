...
```json
{
  "new_status": String,   // "SELLER_CONFIRMED" | "STOCK_PREPARED" | "SHIPPING_STARTED" | "COMPLETED" | "SELLER_CANCELLED"
  "seller_notes": String (可选),
  "cancel_reason": String (可选，仅在new_status为"SELLER_CANCELLED"时使用),
  "cancel_notes": String (可选，仅在new_status为"SELLER_CANCELLED"时使用)
}
```
响应 data 示例（更新后的意向）：
```json
