...
api.interceptors.response.use(response => {
    // 计算API请求时间
    if (response.config && response.config.startTime) {
    ...
    if (error.response && error.response.config && error.response.config.startTime) {
        const duration = Date.now() - error.response.config.startTime;
        performanceMonitor.monitorApiRequest(
