...
3. POST `/api/purchase-intents` 请求体（登录客户）客户提交购买意向（下单），填写商品、数量、联系方式等。
示例请求体与响应体如下：
...
8. POST `/api/seller/products` 请求体（multipart/form-data 或 JSON 引用已上传 URL，已补充必填项与服务器处理）
...
17. POST `/api/media/upload` 请求（multipart/form-data, 无 JSON 请求体）上传媒体资源（图片、视频等），返回文件信息。
    响应 data 示例：
```json
{
  "media_id": Number,
