                ...
                <div class="image-panel">
                  ...
                  <div class="thumbs" v-if="imageList.length">
                    <div
                        ...
                        @click="showImage(idx)"
                    >
                      <img :src="src" alt="缩略图" loading="lazy" />
                    </div>
                  </div>
                        ...
                        <div class="media-grid">
                          ...
                          >
                            <template v-if="isImage(m)">
                              <img 
                                :src="m.media_url || m.image_url || m.url" 
                                alt="媒体图片" 
            ...
            <el-col :xs="24" :sm="24" :md="10" :lg="8">
              <div class="purchase-card">
                <div class="card-inner">
                  ...
                  <div class="quantity-selector" v-if="product.product_status === 'online' && product.stock_quantity > 0">
                    <el-input-number 
...
const addToCart = () => {
  if (!product.value || product.value.product_status !== 'online') {
    ElMessage.error('商品不可购买');
    return;
  }

  if (product.value.stock_quantity != null && product.value.stock_quantity <= 0) {
  ...
  try {
    ...
    const cartRaw = localStorage.getItem('shopping_cart') || '[]';
    const cart = JSON.parse(cartRaw);

    // 检查商品是否已在购物车中
    const existingItemIndex = cart.findIndex(item => 
      item.product_id === (product.value.product_id || product.value.id)
    );
    ...
    const cartItem = {
      ...
      image_url: imageList.value[0] || product.value.image_url,
      stock_quantity: product.value.stock_quantity
    };

    if (existingItemIndex !== -1) {
      // 更新现有商品数量
      cart[existingItemIndex].quantity += purchaseQuantity.value;
      // 确保数量不超过库存
      if (product.value.stock_quantity != null) {
...
@media (max-width: 768px) {
  ...
  .detail-wrap { padding:12px; margin:12px; }
  .title { font-size:18px; }
  .price { font-size:24px; }
  .header { padding:8px 16px; }
  .media-card { width:100%; max-width:200px; }
}

@media (max-width: 480px) {
  .main-image { height:240px; }
  .thumb { width:50px; height:40px; }
  .title { font-size:16px; }
  .price { font-size:22px; }
  .card-inner { padding:14px; }
  .detail-wrap { padding:8px; margin:8px; }
  .actions button { font-size:14px; padding:10px; }
  .media-card { width:100%; max-width:none; }
